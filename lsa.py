...
def summarize(query=None, k=4,url=None):
    ...
    summary = list(dict((v[0],v) for v in sorted(summary, key=lambda summary: summary[1])).values())
...
class LSA(object):
    ...
    def parse(self, doc):
        words = doc.split();
        for w in words:
            w = w.lower().translate(self.ignore_characters)
    ...
    def build(self): # Create count matrix
        self.keys = [k for k in list(self.wdict.keys()) if len(self.wdict[k]) > 1]
        self.keys.sort()
